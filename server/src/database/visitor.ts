...
export async function visitorsCreate(data: validateVisitor) {
  const schemaVisitor = visitorSchema.safeParse(data);

  if (!schemaVisitor.success) { // Return early if the form data is invalid
    return {
  ...
  try {
    const visitors = await pool.query("INSERT INTO visitors SET ?", [
      dataVisitor,
    ]);
    return visitors;
  } catch (error) {
